...
void drawWindow(bool& drawDone, bool& close, ImVec4& clear_color, ImGuiIO& io) {
    while (!drawDone) {
        ...
        if (CardWindow) {
            ImGui::Begin(reinterpret_cast<const char*>(u8"UNO Gaming"), &CardWindow, ImGuiWindowFlags_NoMove);
            ImGui::TextColored(ImColor(102, 204, 255, 255), reinterpret_cast<const char*>(u8"欢迎您！"));
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"更改设置"), &settingsWindow);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"查看玩家"), &showPlayerWindow);

            ImGui::SliderFloat(reinterpret_cast<const char*>(u8"无用的条"), &f, 0.0f, 1.0f);

            ImGui::PushStyleColor(ImGuiCol_CheckMark, ImVec4(0.0f, 1.0f, 0.0f, 1.0f));
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"到"), &showYouTurn);
            ImGui::SameLine(0.0f, 10.0f);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"你"), &showYouTurn);
            ImGui::SameLine(0.0f, 10.0f);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"了"), &showYouTurn);
            ImGui::SameLine(0.0f, 10.0f);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"兵"), &showYouTurn);
            ImGui::SameLine(0.0f, 10.0f);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"贵"), &showYouTurn);
            ImGui::SameLine(0.0f, 10.0f);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"神"), &showYouTurn);
            ImGui::SameLine(0.0f, 10.0f);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"速"), &showYouTurn);
            ImGui::PopStyleColor();

            ImGui::PushStyleColor(ImGuiCol_Button, black);
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, blueActive);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, blueHovered);
            if (ImGui::Button(reinterpret_cast<const char*>(u8"洛神"), ImVec2(250, 35))) {
                ret = 0;
            }
            ImGui::PopStyleColor(3);
        ...
        if (debugWindow) {
            ImGui::Begin(reinterpret_cast<const char*>(u8"界面调试器"), &debugWindow);
            ImGui::TextColored(ImColor(102, 204, 255, 255), reinterpret_cast<const char*>(u8"欢迎您！"));
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"打开调试器"), &showDemoWindow);
            ImGui::Checkbox(reinterpret_cast<const char*>(u8"显示鼠标位置"), &showMouse);
        ...
        if (showMouse) {
            ...
            if (ImGui::BeginTooltip()) {
                ...
                ImGui::Text(reinterpret_cast<const char*>(u8"I am following you around."));
                ImGui::EndTooltip();
            }
            if (ImGui::Begin(
                    " ", nullptr, ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs)) {
                ImGui::End();
            }
        }

        if (talkingWindow) {
            ImGui::Begin(reinterpret_cast<const char*>(u8"聊天界面"), &talkingWindow);
            ImGui::TextColored(ImColor(102, 204, 255, 255),
                               reinterpret_cast<const char*>(u8"Fairy提醒您，请友善沟通交流！"));
            static char inputBuffer[600];
            ImGui::InputText(reinterpret_cast<const char*>(u8"输入框"), inputBuffer, IM_ARRAYSIZE(inputBuffer));
            ImGui::PushStyleColor(ImGuiCol_Button, blue);
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, blueActive);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, blueHovered);
            if (ImGui::Button(reinterpret_cast<const char*>(u8"发送！"))) {
                p_msg = inputBuffer;
        ...
        if (settingsWindow) {
            ImGui::Begin(reinterpret_cast<const char*>(u8"设置"), &settingsWindow);
            ImGui::ColorEdit4(reinterpret_cast<const char*>(u8"自定义背景"),
                              (float*)&clear_color,
                              ImGuiColorEditFlags_::ImGuiColorEditFlags_AlphaBar);
            if (ImGui::Button(reinterpret_cast<const char*>(u8"关闭设置"))) {
                settingsWindow = false;
            }
            ImGui::End();
        }
        if (showPlayerWindow) {
            ImGui::Begin(reinterpret_cast<const char*>(u8"玩家列表"), &showPlayerWindow);
            ImGui::TextColored(ImColor(102, 204, 255, 255),
                               reinterpret_cast<const char*>(u8"玩家列表（别输的太难看bushi）"));
            ...
            if (ImGui::Button(reinterpret_cast<const char*>(u8"关闭列表"))) {
        ...
        if (textWindow) {
            ImGui::Begin(reinterpret_cast<const char*>(u8"文本窗口"), &textWindow, ImGuiWindowFlags_NoMove);
            ImGui::BeginChild(reinterpret_cast<const char*>(u8"ScrollingRegion"),
